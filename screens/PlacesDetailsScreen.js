...
const PlacesDetailsScreen = ({route}) => {
        ...
        <ScrollView className="flex-1 px-4 py-6">
            <View className="relative bg-white shadow-lg">
                <Image 
                    source={
                        {uri :
                            ...
                            data?.photo?.images?.large?.url : 
                            "https://recyclingbalers.s3.amazonaws.com/image/No%20Image.jpg"
                        }
                    }
                ...
                <View className="absolute flex-row inset-x-0 bottom-5 justify-between px-6">
                    <View className="flex-row space-x-2 items-center bg-teal-300 rounded-md">
                        {/* <Text className="text-[12px] font-bold text-gray-100">{data?.price_level}</Text> */}
                        <Text className="text-[20px]">{data?.price}</Text>
                    </View>
                    {data?.open_now_text ? (
                            <>
                                <View className="px-2 py-1 rounded-md bg-teal-300">
                                    <Text className="text-[20px]">{data?.open_now_text}</Text>
                                </View>
                            </>
                        ) : (<></>)
                    }
